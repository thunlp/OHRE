...
def generate_all_info(graph,
                      part_init=None,
                      weight='weight',
                      resolution=1.,
                      randomize=None,
                      random_state=None):
    """Find communities in the graph and return the associated dendrogram, graphs and status
    ...
    status.init(current_graph, weight)
    status_list.append(status.copy())
    graphs_list.append(current_graph.copy())
    while True:
        __one_level(current_graph, status, weight, resolution, random_state)
        # print("modularity", new_mod)
        new_mod = __modularity(status)
        ...
        status.init(current_graph, weight)
        status_list.append(status.copy())
    return partition_list[:], graphs_list[:], status_list[:]
